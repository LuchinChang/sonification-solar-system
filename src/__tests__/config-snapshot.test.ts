...
describe('validateSnapshot (v2)', () => {
  const VALID: ConfigSnapshot = {
    ...
    theme: 'dark',
    shapes: [
      // LEGACY: previously included a 'circle' shape here; non-sweeper types
      // are still accepted by validateSnapshot's runtime Set to preserve
      // loading of pre-quarantine snapshots. Only 'sweeper' renders today.
