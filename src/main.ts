  ...
  if (state.isPlaying && dt > 0) {
    for (const shape of state.shapes) {
      if (shape.type === 'sweeper') {
        // Drive sweeper arm from AudioContext clock
        try {
          if (state.audioInitialized && state.sweepAudioRefTime > 0) {
            const cycleS = 60 / state.cpm;
            const phase  = (state.sweepPhaseAtRef +
              (getAudioContext().currentTime - state.sweepAudioRefTime) / cycleS) % 1;
